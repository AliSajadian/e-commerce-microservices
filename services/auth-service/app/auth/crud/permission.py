...
from ..exceptions import ObjectCreationError, ObjectVerificationError, ObjectNotFoundError
...
class PermissionCRUDs:
    ...
    async def get_permissions(db: AsyncSession, user_id: UUID):
    ...
    async def update(
        self, db: AsyncSession, permission_id: UUID, data
    ):
        """
    ...
    async def delete(self, db: AsyncSession, permission_id: UUID):
        ...
        # 2. If the permission does not exist, return False
        if not permission:
            return False

        # 3. Delete the permission object itself
        await db.delete(permission)

        # 4. Commit the changes
        await db.commit()

        logging.info(f"Successfully deleted permission {permission_id}.")
        return True
